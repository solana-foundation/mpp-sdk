...
pub struct OpenPayload {
    ...
    #[serde(rename = "initMultiDelegateTx", skip_serializing_if = "Option::is_none")]
...
impl OpenPayload {
    ...
    pub fn session_id(&self) -> crate::error::Result<&str> {
        match self.mode {
            SessionMode::Push => self
                .channel_id
                .as_deref()
                .ok_or_else(|| crate::error::Error::Other("push open missing channelId".to_string())),
            SessionMode::Pull => self
                .token_account
                .as_deref()
                .ok_or_else(|| crate::error::Error::Other("pull open missing tokenAccount".to_string())),
        }
    }
    ...
    pub fn deposit_amount(&self) -> crate::error::Result<u64> {
        let raw = match self.mode {
            SessionMode::Push => self
                .deposit
                .as_deref()
                .ok_or_else(|| crate::error::Error::Other("push open missing deposit".to_string()))?,
            SessionMode::Pull => self
                .approved_amount
                .as_deref()
                .ok_or_else(|| crate::error::Error::Other("pull open missing approvedAmount".to_string()))?,
        };
        raw.parse()
...
mod tests {
    ...
    fn session_request_with_splits() {
        let req = SessionRequest {
            ...
            recipient: "rec".to_string(),
            splits: vec![
                SessionSplit { recipient: "s1".to_string(), amount: "100".to_string() },
                SessionSplit { recipient: "s2".to_string(), amount: "200".to_string() },
            ],
    ...
    fn open_payload_push_session_id_and_deposit() {
        let p = OpenPayload::push("chan1".to_string(), "2000000".to_string(), "s".to_string(), "sig".to_string());
    ...
    fn open_payload_pull_session_id_and_deposit() {
        let p = OpenPayload::pull("tokacct".to_string(), "3000000".to_string(), "wallet1".to_string(), "s".to_string(), "sig".to_string());
    ...
    fn open_payload_push_roundtrip_json() {
        let p = OpenPayload::push("chan1".to_string(), "1000000".to_string(), "signer1".to_string(), "txsig".to_string());
    ...
    fn open_payload_pull_roundtrip_json() {
        let p = OpenPayload::pull("tokacct".to_string(), "5000000".to_string(), "wallet1".to_string(), "signer1".to_string(), "approvesig".to_string());
        let json = serde_json::to_string(&p).unwrap();
    ...
    fn open_payload_missing_mode_fails_deserialization() {
        ...
        let json = r#"{"channelId":"chan1","deposit":"1000","authorizedSigner":"s","signature":"sig"}"#;
    ...
    fn voucher_data_canonical_bytes_deterministic() {
        ...
        assert_eq!(data.canonical_bytes().unwrap(), data.canonical_bytes().unwrap());
    }

    #[test]
    fn voucher_data_canonical_bytes_differs_by_cumulative() {
        let a = VoucherData { channel_id: "c".to_string(), cumulative: "100".to_string(), nonce: None };
        let b = VoucherData { channel_id: "c".to_string(), cumulative: "200".to_string(), nonce: None };
