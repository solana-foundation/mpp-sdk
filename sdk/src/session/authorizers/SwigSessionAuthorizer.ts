...
export class SwigSessionAuthorizer implements SessionAuthorizer {
  ...
  getCapabilities(): AuthorizerCapabilities {
    return {
      ...
      ...(this.policy.spendLimit
        ? { maxCumulativeAmount: this.policy.spendLimit }
        : {}),
      ...(this.policy.depositLimit
        ? { maxDepositAmount: this.policy.depositLimit }
        : {}),
  ...
  async authorizeOpen(input: AuthorizeOpenInput): Promise<AuthorizedOpen> {
    ...
    if (this.depositLimit !== undefined && deposit > this.depositLimit) {
      throw new Error(
        `Open deposit exceeds depositLimit (${this.depositLimit.toString()})`,
      )
    }
    ...
    this.channels.set(input.channelId, {
      ...
      ...(session.swigRoleId !== undefined
        ? { swigRoleId: session.swigRoleId }
        : {}),
    })
  ...
  async authorizeUpdate(input: AuthorizeUpdateInput): Promise<AuthorizedUpdate> {
    ...
    if (this.spendLimit !== undefined && cumulativeAmount > this.spendLimit) {
      throw new Error(
        `Cumulative amount exceeds spendLimit (${this.spendLimit.toString()})`,
      )
    }
  ...
  async authorizeTopup(input: AuthorizeTopupInput): Promise<AuthorizedTopup> {
    ...
    if (this.depositLimit !== undefined && nextDeposited > this.depositLimit) {
      throw new Error(
        `Topup exceeds depositLimit (${this.depositLimit.toString()})`,
      )
    }
  ...
  async authorizeClose(input: AuthorizeCloseInput): Promise<AuthorizedClose> {
    ...
    if (this.spendLimit !== undefined && finalCumulativeAmount > this.spendLimit) {
      throw new Error(
        `Final cumulative amount exceeds spendLimit (${this.spendLimit.toString()})`,
      )
    }
  ...
  private requireActiveSessionSigner(
    ...
    if (!this.sessionSigner || this.sessionStartedAtMs === null) {
      throw new Error(
        `No active Swig session key for channel ${channelId}. Call authorizeOpen first.`,
      )
    }

    if (this.isSessionExpired()) {
      throw new Error(
        'Swig session key has expired. Re-open the channel to create a fresh session key.',
      )
    }
  ...
  private async ensureSessionSignerForOpen(
    input: AuthorizeOpenInput,
    ...
    const existingResult = this.wallet.getSessionKey
      ? await this.wallet.getSessionKey()
      : null
    if (existingResult) {
    ...
    const createdResult = await this.wallet.createSessionKey({
      ...
      ...(this.policy.depositLimit
        ? { depositLimit: this.policy.depositLimit }
        : {}),
      channelId: input.channelId,
  ...
  private async resolveTopupTx(input: AuthorizeTopupInput): Promise<string> {
    if (!this.buildTopupTx) {
      throw new Error(
        'SwigSessionAuthorizer requires `buildTopupTx` to authorize topup requests',
      )
    }
  ...
  private setSessionState(state: SessionSignerState) {
    ...
    this.sessionRoleId =
      state.swigRoleId ?? this.wallet.swigRoleId ?? null
  ...
  private async assertPolicyAppliedOnChain(
    input: AuthorizeOpenInput,
    session: SessionSignerState,
  ) {
    ...
    if (!actions) {
      throw new Error(
        `Swig role ${role.id} does not expose action metadata for policy validation`,
      )
    }
  ...
  private resolveSessionRole(
    ...
    const preferredRoleId =
      session.swigRoleId ?? this.sessionRoleId ?? this.wallet.swigRoleId
    ...
    if (
      ...
      throw new Error(
        `Unable to locate Swig role ${preferredRoleId} for session key ${session.signer.address}`,
      )
    }
  ...
  private assertRoleAllowsProgram(
    ...
    if (!actions.canUseProgram(channelProgram)) {
      throw new Error(
        `Swig role ${roleId} does not allow channel program ${channelProgram}`,
      )
    }
  }
  ...
  private resolveOnChainSpendLimit(
    ...
    if (input.asset.kind === 'spl') {
      if (!input.asset.mint) {
        throw new Error(
          'asset.mint is required for SPL session policy validation',
        )
      }

      if (!actions.tokenSpendLimit) {
        throw new Error(
          'Swig role does not expose tokenSpendLimit() for SPL policy validation',
        )
      }
    ...
    if (!actions.solSpendLimit) {
      throw new Error(
        'Swig role does not expose solSpendLimit() for SOL policy validation',
      )
    }
...
function normalizeSessionSignerState(
  ...
  source: 'getSessionKey' | 'createSessionKey',
): SessionSignerState {
  ...
  if (!value || typeof value !== 'object') {
    throw new Error(
      `Swig wallet ${source}() must return a signer or an object containing { signer }`,
    )
  }
  ...
  if (!isSignerLike(signer)) {
    throw new Error(
      `Swig wallet ${source}() returned an invalid signer`,
    )
  }
  ...
  if (openTx !== undefined && typeof openTx !== 'string') {
    throw new Error(
      `Swig wallet ${source}() returned invalid openTx; expected string`,
    )
  }
  ...
  if (swigRoleIdRaw !== undefined) {
    if (typeof swigRoleIdRaw !== 'number') {
      throw new Error(
        `Swig wallet ${source}() returned invalid swigRoleId; expected a non-negative integer`,
      )
    }
