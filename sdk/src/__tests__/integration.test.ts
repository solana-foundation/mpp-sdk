...
async function startSessionHarness(
  overrides: Partial<SessionServerParameters> = {},
...
async function getSessionChannel(
  store: Store.Store,
  channelId: string,
) {
...
async function createSwigHarness(parameters: {
  ...
  const createSwigInstruction = await getCreateSwigInstruction({
    ...
    actions: Actions.set()
      .manageAuthority()
      .programAll()
      .solLimit({ amount: spendLimitLamports })
      .get(),
    authorityInfo: createEd25519SessionAuthorityInfo(
      walletSigner.address,
      BigInt(sessionTtlSeconds),
    ),
  })
...
test('e2e: session close action returns 204 and next request opens a new channel', async () => {
  ...
  try {
    ...
    const reopenedChannel = await getSessionChannel(
      harness.store,
      reopenedReceipt.reference,
    )
...
test('e2e: session swig_session mode uses on-chain setup and enforces spend limit', async () => {
  ...
  try {
    ...
    const withinLimitSignature = await swig.spendFromSwig(
      500n,
