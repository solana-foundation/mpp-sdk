...
async function buildOpenCredential(
  options: OpenCredentialOptions,
): Promise<any> {
...
async function buildUpdateCredential(
...
async function buildCloseCredential(
  options: CloseCredentialOptions,
): Promise<any> {
  const voucher =
    options.voucher ??
    (await buildSignedVoucher({
