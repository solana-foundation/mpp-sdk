...
def test_fortune_returns_402(client: httpx.Client) -> None:
    resp = client.get("/fortune")
    assert resp.status_code == 402


def test_fortune_has_www_authenticate(client: httpx.Client) -> None:
    resp = client.get("/fortune")
    assert "www-authenticate" in resp.headers


def test_challenge_has_payment_scheme(client: httpx.Client) -> None:
    resp = client.get("/fortune")
    www_auth = resp.headers["www-authenticate"]
    assert www_auth.startswith("Payment ")


def test_challenge_has_required_fields(client: httpx.Client) -> None:
    resp = client.get("/fortune")
...
def test_challenge_method_is_solana(client: httpx.Client) -> None:
    resp = client.get("/fortune")
    challenge = parse_www_authenticate(resp.headers["www-authenticate"])
    assert challenge["method"] == "solana"


def test_challenge_intent_is_charge(client: httpx.Client) -> None:
    resp = client.get("/fortune")
    challenge = parse_www_authenticate(resp.headers["www-authenticate"])
    assert challenge["intent"] == "charge"


def test_challenge_request_is_valid_base64url_json(client: httpx.Client) -> None:
    resp = client.get("/fortune")
    challenge = parse_www_authenticate(resp.headers["www-authenticate"])
...
def test_challenge_request_has_method_details(client: httpx.Client) -> None:
    resp = client.get("/fortune")
    challenge = parse_www_authenticate(resp.headers["www-authenticate"])
    decoded = json.loads(base64url_decode(challenge["request"]))
...
def test_challenge_has_cache_control(client: httpx.Client) -> None:
    resp = client.get("/fortune")
